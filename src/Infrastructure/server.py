...
class backgroundTime():    
    def schedule_time(conn):
        schedule_manager = BackgroundScheduler()
        schedule_manager.remove_all_jobs()
        schedule_manager.add_job(AddInvoice.addInvoice, args=[conn], trigger=IntervalTrigger(hours=3)) # A cada 3 horas fara 8 a 12 faturas
        #schedule_manager.add_job(AddInvoice.addInvoice, args=[conn], trigger=IntervalTrigger(seconds=30)) # A cada 30 segs fara 8 a 12 faturas(mais facil para testar)
